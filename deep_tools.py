...
class DataGenerator:
    ...
    def next_batch(self,strategy='local',batch_size=1000):

        if strategy=='local':
            seq_length=self.local_random_list[np.random.randint(len(self.local_random_list))]
            batch_size=batch_size//(seq_length-14)+1
        else:
            seq_length=self.online_random_list[np.random.randint(len(self.online_random_list))]
            batch_size=batch_size//(seq_length-7)+1
    ...
    def get_set(self,strategy='local',usage='train'):
        if strategy=='local':
            if usage=='train':
            ...
            if usage=='train':
                test_list=self.online_train_list
            else:
                test_list=self.online_test_list
